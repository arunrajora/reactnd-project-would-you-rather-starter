...
const getUnAnsweredQuestions = (questions, user) =>
  sortQuestionsByTimestamp(
    questions
      .filter(({ id }) => !isAnswered(id, user))
      .map(question => question)
  );
const formatQuestions = (questions, users, authedUser) =>
  questions.map(({ id, author, optionOne, optionTwo, timestamp }) => ({
    id,
    timestamp,
...
const mapStateToProps = ({ questions, authedUser, users }) => ({
  answeredQuestions: formatQuestions(
    getAnsweredQuestions(Object.values(questions), users[authedUser]),
    users,
    authedUser
  ),
  unansweredQuestions: formatQuestions(
    getUnAnsweredQuestions(Object.values(questions), users[authedUser]),
    users,
    authedUser
  )
});
